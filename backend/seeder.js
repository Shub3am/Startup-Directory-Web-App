...
db.run(`CREATE TABLE IF NOT EXISTS ${Table}(ID INTEGER PRIMARY KEY AUTOINCREMENT, Name TEXT NOT NULL, Description TEXT ,Founded TEXT NOT NULL, Industry TEXT NOT NULL ON CONFLICT REPLACE DEFAULT 'Unknown', Funding_Amount INTEGER NOT NULL ON CONFLICT REPLACE DEFAULT 'Unknown', Investor TEXT NOT NULL ON CONFLICT REPLACE DEFAULT 'Unknown', Investment_Type TEXT NOT NULL ON CONFLICT REPLACE DEFAULT 'Unknown', City TEXT NOT NULL ON CONFLICT REPLACE DEFAULT 'Unknown' )`, (result)=> {
